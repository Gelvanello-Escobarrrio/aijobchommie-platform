-- =====================================
-- 🚀 AI JOB CHOMMIE PLATFORM - SUPABASE SQL SCHEMA
-- Complete database schema for world-class SaaS platform
...
CREATE INDEX idx_job_applications_tracking_number ON job_applications(tracking_number);
...
CREATE INDEX idx_file_uploads_is_processed ON file_uploads(is_processed);
...
-- Insert sample South African provinces data (for reference)
-- This will be handled by application logic, but useful for development

-- Insert sample job categories
INSERT INTO jobs (
  title, company, location, province, city, description, job_type, 
...
(
  ...
  'Gauteng',
  'Johannesburg',
  'Join our professional cleaning team servicing office buildings in Sandton area. We provide all equipment and training.',
  'full-time',
  'Cleaning & Domestic',
  'entry-level',
  3000,
...
(
  ...
  'KwaZulu-Natal',
  'Durban',
  'Busy restaurant looking for kitchen assistant to help with food preparation, dishwashing, and general kitchen duties.',
  'part-time',
  'Food Service',
  'entry-level',
  2800,
